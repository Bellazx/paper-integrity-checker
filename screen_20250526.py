#!/usr/bin/env python3
"""Run initial screening on 20250526 batch papers. Writes results to yujing_quanliang table."""
import json
import logging
import sys
import time
...
log = logging.getLogger("screen-20250526")

BASE = Path(__file__).resolve().parent
INPUT_BASE = BASE / "data" / "input" / "20250526"
OUTPUT_BASE = BASE / "data" / "output"
...
def get_paper_dirs():
    ...
    for d in sorted(INPUT_BASE.iterdir()):
        if d.is_dir() and any(d.glob("*.pdf")):
            report = OUTPUT_BASE / d.name / "report.json"
            if report.exists():
                continue
            dirs.append(d)
    return dirs
...
def _insert_quanliang(findings: dict):
...
def process_one(paper_dir: Path) -> dict:
    ...
    try:
        ...
        risk_level = _insert_quanliang(findings)
...
def main():
    ...
    output_path = OUTPUT_BASE / "20250526_screening_results.json"
...
if __name__ == "__main__":
    high_risk = main()
    sys.exit(0 if not high_risk else min(len(high_risk), 255))
