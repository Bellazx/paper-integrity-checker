...
2. 检查首轮复核是否真正覆盖每条 HIGH finding、确定性数据发现、主图图文一致性核查。
...
def _ensure_evidence_bundle(doi, report_json_path, input_dir):
    ...
    out = os.path.join(out_dir, doi.replace("/", "_") + "_evidence.json")
...
async def run_review_single(
    ...
    if not _valid_review_result(result1):
    ...
    if not _valid_review_result(validated1):
    ...
    if not _valid_review_result(result2):
    ...
    if not _valid_review_result(validated2):
    ...
    if not _valid_review_result(judge_result):
