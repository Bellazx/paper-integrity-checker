#!/usr/bin/env python3
"""Run initial screening on Zhang Wanbin papers with supplementary data.
Writes results to yujing_wanbin table (NOT yujing_quanliang)."""
import json
import logging
import sys
import threading
...
log = logging.getLogger("wanbin-screen")
...
def _insert_wanbin(findings: dict):
...
def process_one(paper_dir: Path) -> dict:
    ...
    try:
        ...
        risk_level = _insert_wanbin(findings)
...
def main():
    paper_dirs = get_supp_dirs()
    log.info("Found %d papers with supplementary data to screen", len(paper_dirs))
